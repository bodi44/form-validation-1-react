...
export const validateEmail = (value, form) => {
  ...
  } else if (!phoneRegExp.test(form.values.phoneNumber) && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(value)) {
    error = 'Invalid email';
  }
...
export const validatePhoneNumber = (value, values) => {
  ...
  if (!value && !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
    error = 'Required!';
  } else if (value && /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
    if (!phoneRegExp.test(value)) {
      error = 'Invalid phone!';
    }
  } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email) && !phoneRegExp.test(value)) {
    error = 'Invalid phone!';
  }
